...
const agregar = async(req, res) =>{
    try {
        ...
        const alumno = await alumnosModel.find({$and: [{id},{idGrupo}]});
        if(alumno.length == 0) {
            ...
            return res.status(400).send(`Ya existe el alumno con el mismo id`);
        }
    } catch (error) {
...
const modificar = async (req,res)=>{
    try {
        ...
        const registro = await alumnosModel.find({$and: [{id},{idGrupo}]});
